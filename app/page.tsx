...
export default function PranaIndexOfficial() {
  ...
  const startAtmosphere = () => {
    ...
    if (!rainAudio.current) {
      ...
      rainAudio.current.loop = true; rainAudio.current.volume = 0.4;
    }
  ...
  const playRev = (startF: number, endF: number, dur: number, vol: number) => {
    ...
    osc1.connect(g); g.connect(audioCtx.current.destination);
  ...
  const calculateFinalScore = () => {
    ...
    const consistency = Math.max(0, 100 - (Math.sqrt(variance) / 6));

    const reflex = Math.max(0, 100 - ((p2TotalTime / 5) - 250) / 8);
    const focus = (p3Hits.reduce((a,b)=>a+b, 0) / (p3Hits.length || 1));

    const final = (consistency * 0.4) + (reflex * 0.3) + (focus * 0.3);
    setFinalScore(Math.floor(Math.min(100, final)));
    playTransitionSound();
    setScreen('result');
  };
  ...
  const startP1 = () => {
    ...
    const timer = setInterval(() => {
      ...
      if (time <= 0) { clearInterval(timer); playTransitionSound(); setScreen('p2'); setP2StartTime(Date.now()); }
    }, 1000);
  };
  ...
  const startRegulation = () => {
    ...
    setRegPhase('INHALE'); let time = 5.0; playRev(100, 450, 5, 0.1);
    const inhaleInt = setInterval(() => {
      time -= 0.1; setRegTimer(time);
      if (time <= 0.05) {
        clearInterval(inhaleInt); setRegPhase('HOLD'); playRev(450, 450, 3, 0.02); let hTime = 3.0;
        const holdInt = setInterval(() => {
          hTime -= 0.1; setRegTimer(hTime);
          if (hTime <= 0.05) {
            clearInterval(holdInt); setRegPhase('EXHALE'); playRev(450, 100, 5, 0.1); let eTime = 5.0;
            const exhaleInt = setInterval(() => {
              eTime -= 0.1; setRegTimer(eTime);
              if (eTime <= 0.05) { clearInterval(exhaleInt); setRegCycles(c => c + 1); setRegPhase('READY'); }
            }, 100);
          }
        }, 100);
      }
  ...
  useEffect(() => {
    if (screen === 'reg' && regPhase === 'READY' && regCycles < 3) setTimeout(startRegulation, 1000);
  }, [screen, regPhase, regCycles]);

  // --- 6. SHARING & CANVAS ---
  const downloadScoreImage = () => {
    const canvas = document.createElement('canvas');
    canvas.width = 1080; canvas.height = 1080; const ctx = canvas.getContext('2d')!;
    ...
    const img = new Image(); img.crossOrigin = "anonymous"; img.src = '/gold-pi-logo.png'; 
    img.onload = () => {
      const imgW = 200; const imgH = (img.height / img.width) * imgW;
      ctx.drawImage(img, 540 - imgW/2, 120, imgW, imgH);
      ctx.textAlign = 'center'; ctx.fillStyle = '#D4AF37'; ctx.font = '900 40px Inter, sans-serif'; ctx.letterSpacing = '10px'; ctx.fillText('PRANA INDEX', 540, 380);
      ctx.font = '700 45px Inter, sans-serif'; ctx.fillStyle = 'white'; ctx.fillText(userName.toUpperCase(), 540, 470);
      ctx.font = '700 24px Inter, sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.5)'; ctx.fillText('STRESS SCORE', 540, 530);
      ctx.font = '900 350px Inter, sans-serif'; ctx.fillStyle = '#D4AF37'; ctx.fillText(finalScore.toString(), 540, 840);
      ctx.font = '900 40px Inter, sans-serif'; ctx.fillStyle = 'white'; ctx.fillText(getVerdict().title, 540, 940);
      ctx.font = '400 24px Inter, sans-serif'; ctx.fillStyle = 'rgba(212,175,55,0.6)'; ctx.fillText('www.pranaindex.com', 540, 1020);
      const link = document.createElement('a'); link.download = `PI_Score_${userName}.png`; link.href = canvas.toDataURL('image/png'); link.click();
    };
  };
      ...
      <div style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '20px' }}>
        ...
        {screen === 'landing' && (
          <div style={{ textAlign: 'center', width: '100%', maxWidth: '320px' }}>
            ...
            <input placeholder="ENTER YOUR NAME" value={userName} onChange={e => setUserName(e.target.value)} style={{ width: '100%', padding: '18px', borderRadius: '50px', border: '1px solid #D4AF37', background: 'rgba(255,255,255,0.05)', color: 'white', textAlign: 'center', marginBottom: '20px', fontSize: '14px', fontWeight: 900 }} />
        ...
        {screen === 'p1' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <div onPointerDown={() => { setIsTapping(true); setP1Taps([...p1Taps, Date.now()]); playTone(200, 'sine', 0.1, 0.2); }} onPointerUp={() => setIsTapping(false)} style={{ width: '200px', height: '200px', backgroundColor: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transform: isTapping ? 'scale(0.92)' : 'scale(1)', transition: '0.1s', boxShadow: isTapping ? '0 0 60px white' : '0 0 20px rgba(255,255,255,0.2)' }}>
              <img src="/gold-pi-logo.png" style={{ width: '120px' }} />
            </div>
            <p style={{ position: 'absolute', bottom: '80px', width: '100%', left: 0, textAlign: 'center', fontSize: '11px', opacity: 0.7, letterSpacing: '1px' }}>MAINTAIN A STEADY RHYTHM</p>
          </div>
        )}
        ...
        {screen === 'p2' && (
          <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
             ...
             <button onPointerDown={() => { playTone(800, 'triangle', 0.1, 0.2); if(p2Count + 1 >= 5) { setP2TotalTime(Date.now() - p2StartTime); playTransitionSound(); setScreen('p3'); } else { setP2Count(c => c + 1); targetPos.current = { left: Math.random()*70+15+'%', top: Math.random()*70+15+'%' }; } }} style={{ position: 'absolute', left: targetPos.current.left, top: targetPos.current.top, width: '80px', height: '80px', borderRadius: '50%', backgroundColor: '#D4AF37', color: 'black', fontWeight: 900, border: 'none', cursor: 'pointer' }}>π</button>
        ...
        {screen === 'p3' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ width: '280px', height: '280px', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' }}>
                ...
                <button onPointerDown={() => { const newHits = [...p3Hits, pulseSize]; setP3Hits(newHits); if (pulseSize > 85) playTone(1200, 'sine', 0.2, 0.3); else playTone(150, 'sawtooth', 0.2, 0.2); if (newHits.length >= 3) calculateFinalScore(); }} style={{ zIndex: 10, width: '110px', height: '110px', borderRadius: '50%', backgroundColor: 'black', border: '3px solid #D4AF37', color: '#D4AF37', fontWeight: 900, cursor: 'pointer' }}>TAP PEAK</button>
            </div>
            <p style={{ position: 'absolute', bottom: '80px', width: '100%', left: 0, textAlign: 'center', fontSize: '11px', opacity: 0.7, letterSpacing: '1px' }}>HIT THE PEAK WHEN THE PULSE TURNS GREEN</p>
          </div>
        )}
        ...
        {screen === 'result' && (
          <div style={{ textAlign: 'center', animation: 'fadeIn 1s', maxWidth: '380px', width: '100%' }}>
            ...
            <div style={{ backgroundColor: 'rgba(255,255,255,0.05)', padding: '20px', borderRadius: '25px', border: '1px solid #ffffff11', marginBottom: '30px' }}>
               <p style={{ color: 'white', fontWeight: 900, marginBottom: '5px' }}>{userName.toUpperCase()}</p>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '30px' }}>
               <button onClick={downloadScoreImage} style={{ flex: 1, padding: '15px', borderRadius: '30px', border: '1px solid #D4AF37', color: '#D4AF37', background: 'none', fontWeight: 900, cursor: 'pointer' }}>📥 Save Card</button>
        ...
        {screen === 'reg' && (
          <div style={{ textAlign: 'center' }}>
            ...
            {regPhase === 'DONE' && <button onClick={() => window.location.reload()} style={{ color: '#39FF14', fontWeight: 900, marginTop: '30px', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>RESTART GAME</button>}
          </div>
        )}
      </div>
      <style jsx global>{`
